...
const ConfirmarFechamento = ({ navigation }) => {
  ...
  const handleAdicionalIncrement = (adicionalId) => {
    setAdicionais(
      adicionais.map(adicional =>
        adicional.id === adicionalId ? { ...adicional, quantity: adicional.quantity + 1 } : adicional
      )
    );
  };
  ...
  const handleAdicionalDecrement = (adicionalId) => {
    setAdicionais(
      adicionais.map(adicional =>
        adicional.id === adicionalId && adicional.quantity > 0 ? { ...adicional, quantity: adicional.quantity - 1 } : adicional
      )
    );
  };
    ...
    <View style={styles.modalContainer}>
      ...
      <View style={styles.totalRow}>
        ...
        <View style={styles.buttonGroup}>
          <TouchableOpacity style={styles.adicionaisButton} onPress={() => setModalVisible(true)}>
